...
final class MultiMatchQuizArgs {
...
class MultiMatchQuizScreen extends StatefulWidget {
  ...
  static Route<dynamic> route(RouteSettings settings) {
    final args = settings.arguments! as MultiMatchQuizArgs;
...
class _MultiMatchQuizScreenState extends State<MultiMatchQuizScreen>
  ...
  late final quizDuration =
      context.read<SystemConfigCubit>().quizTimer(QuizTypes.multiMatch);
  ...
  void timerListener(AnimationStatus status) {
    if (status == AnimationStatus.completed) {
      ...
      context
          ...
          .timeOutOnQuestion(currQuestionIndex);
    }
  }
  ...
  late final questions =
      context.read<MultiMatchFetchQuestionsCubit>().questions;
  ...
  void _toggleOptionSelection(String id) {
    ...
    context
        .read<MultiMatchFetchQuestionsCubit>()
        .updateSelectedOptions(currQue.id, id);
  }
  ...
  void _onReorder(List<String> selectedOptionsIds) {
    ...
    context
        .read<MultiMatchFetchQuestionsCubit>()
        .onReorderOptions(currQue.id, selectedOptionsIds);
  }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<MultiMatchFetchQuestionsCubit,
    ...
    Future<void> onTap() async {
      if (currQue.hasSubmittedAnswers) {
        ...
        if (currQue.answerType == MultiMatchAnswerType.multiSelect) {
          ...
          isCorrectlyAnswered =
              listEquals<String>(correctAnswersIds, currQue.submittedIds);
        }
        ...
        if (isCorrectlyAnswered) {
          unawaited(playSound(correctAnswerSoundTrack));
        } else {
          unawaited(playSound(wrongAnswerSoundTrack));
        }
...
class _MultiMatchQuestionsViewState extends State<MultiMatchQuestionsView> {
  ...
  Widget _buildQuestionContainer(
        ...
        if (question.answerType == MultiMatchAnswerType.multiSelect) {
          isCorrectlyAnswered =
              correctAnswersIds.length == question.submittedIds.length &&
                  correctAnswersIds
                      .toSet()
                      .containsAll(question.submittedIds.toSet());
        } else {
          isCorrectlyAnswered =
              listEquals<String>(correctAnswersIds, question.submittedIds);
        }
    ...
    return Container(
      ...
      width: context.width * UiUtils.questionContainerWidthPercentage,
      height:
          context.height * (UiUtils.questionContainerHeightPercentage - 0.045),
      child: showContent
          ? SlideTransition(
...
class _MultiMatchAnswerOptionsState extends State<MultiMatchAnswerOptions> {
  ...
  late final margin =
      EdgeInsets.only(bottom: widget.constraints.maxHeight * .015);

  late final updatedOptions = List.generate(
  ...
  Widget build(BuildContext context) {
    if (widget.answerType == MultiMatchAnswerType.sequence) {
      ...
      return Column(
        ...
        children: widget.answerOptions.map(
          (option) {
